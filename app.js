...
app.post('/auth', function(req, res){
  ...
  if(name && password){
    conn.query('SELECT * FROM users WHERE name = ? AND password = ?', [name, password], // main connection to the data base table
...
//Users can access this only if they are loged in
...
app.post('/authMPs', function(req, res, next){
    var id = req.body.id;
...
app.get('/logout',(req,res) => { //route to logout from members only page
    req.session.destroy();
    res.redirect('/');
});
